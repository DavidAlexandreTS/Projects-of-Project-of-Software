...
public class Homepage {
	...
	public static void main(String args[]) {
		...
		while (true) {
			...
			if (choice.equals("1")) {
				System.out.println(
						"To create an account please provide the data below(Login, Username and Password respectively");
				while (true) {
					login = adc.nextLine();
					...
					while (i < acount.size()) {
						if (acount.get(i).getLogin().equals(login)) {
							System.out.println(
									"We already have a registered user with this Login. How about trying another one?");
							est = 1;
							break;
						}
						i++;
				...
				System.out.println("Hello, to create a profile you need to have an iFace account,"
						...
						+ " if you already have and just want to Create a Profile Type 2"
						+ " and if you want edit a Profile Type 3.");
				...
				if (newchoice.equals("1")) {
					...
					System.out
							.println("Enter the date of your birthday, a small biography and its address respectively");
					...
					while (true) {
						...
						if (yes != 1) {
							System.out.println(
									"The account you reported does not exist or is incorrect, please try again.");
						} else {
							break;
						}
					}
					...
					System.out.println(
							"To change the Birthday press 1, the Biography press 2, the Addres press 3 and for all press 4");
				...
				if (opt.equals("1")) {
					...
					if (acount.get(idtotal - 1).getRequests().size() != 0) {
						...
						int i = 0;
						while (i < auxin) {
							System.out.println("The User " + acount.get(idtotal - 1).getRequests().get(i).getSent()
									+ " want add you has a friend. Digite Yes to aceppt and No to decline.");
							answer = adc.nextLine();
							...
							if (answer.equals("Yes")) {
								 ...
								 * //System.out.println("The problem is: " + idtotal);
								 * if(acount.get(j).getUsername().equals(acount.get(idtotal -
								 * 1).getRequests().get(j).getSent())) { idaux = acount.get(j).getIdacc(); } }
								 */
